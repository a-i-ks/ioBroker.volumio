...
class Volumio extends utils.Adapter {
    ...
    private async handleConnectionChange(connected: boolean): Promise<void> {
        this.log.info(`Connection to Volumio ${connected ? 'established' : 'lost'}`);
    ...
    private async onReady(): Promise<void> {
        ...
        this.volumioClient.onConnectionChange(this.handleConnectionChange.bind(this));
    ...
    private async volumeUp(): Promise<void> {
        ...
        try {
            ...
            const currentVolume = (state?.val as number) || 0;
